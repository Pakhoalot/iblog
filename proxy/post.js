 ...
 * @param {*} callback 回调
 */
function getPageCount(params, callback) {
    ...
    redisClient.getItem(cache_key, (err, pageCount) => {
        ...
        const query = getPostQuery(params);
 ...
 * @param {*} callback 回调函数
 */
function getPostByAlias(alias, callback) {
 ...
 * @param callback 回调函数
 */
function getArticlesCount(params, callback) {
    const query = getArticlesQuery(params);
    postModel.count(query, (err, count) => {
        if (err) {
            return callback(err);
 ...
 * @param callback 回调函数
 */
function checkAlias(alias, articleId, callback) {
    postModel.findOne({ Alias: alias }, (err, article) => {
        if (err) {
            return callback(err);
        }
